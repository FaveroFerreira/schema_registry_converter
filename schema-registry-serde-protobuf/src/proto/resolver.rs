...
use crate::proto::{
    Enum, Extend, Extensions, Field, FieldType, Message, Modifier, OneOf, ProtoSchema, Syntax,
};
...
fn frequency(input: &str) -> IResult<&str, ModifierToken> {
    alt((
        value(ModifierToken::Optional, tag("optional")),
        value(ModifierToken::Repeated, tag("repeated")),
        value(ModifierToken::Required, tag("required")),
    ))(input)
}
...
impl TryFrom<(Syntax, FieldType, Option<ModifierToken>)> for Modifier {
    type Error = &'static str;

    fn try_from(value: (Syntax, FieldType, Option<ModifierToken>)) -> Result<Self, Self::Error> {
        match value {
            (Syntax::Proto2, FieldType::Map(..), _) => Ok(Modifier::Map),
            (Syntax::Proto2, _, Some(ModifierToken::Required)) => Ok(Modifier::Required),
            (Syntax::Proto2, _, Some(ModifierToken::Optional)) => Ok(Modifier::Optional),
            (Syntax::Proto2, _, Some(ModifierToken::Repeated)) => Ok(Modifier::Repeated),
            (Syntax::Proto2, _, None) => Ok(Modifier::Optional),
            (Syntax::Proto3, FieldType::Map(..), _) => Ok(Modifier::Map),
            (Syntax::Proto3, _, Some(ModifierToken::Required)) => Ok(Modifier::Optional),
            (Syntax::Proto3, _, Some(ModifierToken::Optional)) => Ok(Modifier::Optional),
            (Syntax::Proto3, _, Some(ModifierToken::Repeated)) => Ok(Modifier::Repeated),
            (Syntax::Proto3, _, None) => Ok(Modifier::Required),
        }
    }
}
...
where
    F: Fn((Syntax, FieldType, Option<ModifierToken>)) -> Result<Modifier, &'static str>,
{
    move |input| -> IResult<&str, Field> {
        map_res(
            ...
            |(freq, typ, (name, number), key_vals)| {
                Ok::<Field, &str>(Field {
                    name,
                    modifier: freq_func((syntax, typ.clone(), freq))?,
                    number,
...
fn oneof_message_field(syntax: Syntax) -> impl FnMut(&str) -> IResult<&str, Field> {
    field_generic(syntax, |(syntax, _, _)| {
        Ok(match syntax {
            Syntax::Proto2 => Modifier::Required,
            Syntax::Proto3 => Modifier::Optional,
        })
    })
}
...
fn enumerator(input: &str) -> IResult<&str, Enum> {
    map_res(
        terminated(
            pair(
...
pub fn proto_resolver<'a>(
    input: &'a str,
) -> IResult<&'a str, ProtoSchema, nom::error::Error<String>> {
    let (_, stx) = scan_syntax(input).unwrap_or_default();

    let parser = move |input: &'a str| -> IResult<&'a str, ProtoSchema, nom::error::Error<&str>> {
        map(
            many0(alt((
                map(syntax, Event::Syntax),
                map(import, Event::Import),
                map(package, Event::Package),
                map(message(stx), Event::Message),
                map(enumerator, Event::Enum),
                map(extend(stx), Event::Extend),
                value(Event::Ignore, option_ignore),
                value(Event::Ignore, br),
            ))),
            |events| {
                let mut desc = ProtoSchema::default();

                for event in events {
                    match event {
                        Event::Syntax(s) => desc.syntax = s,
                        Event::Import(i) => desc.imports.push(i),
...
mod test {
    ...
    fn assert_complete<T>(parse: IResult<&str, T>) -> Result<T, &str> {
        let (rem, obj) = parse.expect("valid parse");
        result_assert_eq(rem, "", Some("expected no trailing data"))?;
        Ok(obj)
    }

    fn assert_desc(msg: &str) -> Result<ProtoSchema, &str> {
        let (rem, obj) = proto_resolver(msg).expect("valid parse");
        result_assert_eq("", rem, Some("expected no trailing data"))?;
        Ok(obj)
    }
    ...
    fn test_comments() {
        ...
        let msg = r#"
            ...
            "#;
        let desc = proto_resolver(msg).unwrap().1;
    ...
    fn test_extend() {
        ...
        let desc = proto_resolver(msg).unwrap().1;
        assert_eq!(1, desc.messages.len());
        assert_eq!(1, desc.extends.len());
        let extend = &desc.extends[0];
    ...
    fn enum_semicolon() {
        ...
        assert_eq!(
            ...
            desc.messages[0].fields[0].r#type
        );
