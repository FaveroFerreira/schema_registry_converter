...
pub enum ProtoResolverError {
    #[error("Could not retrieve name for index: {0:?}")]
...
pub fn resolve_name(
    ...
    index: &[i32],
) -> Result<Arc<String>, ProtoResolverError> {
    match resolver.find_name(index) {
        Some(n) => Ok(n),
        None => Err(ProtoResolverError::UnresolvableIndex(index.to_vec())),
    }
}
...
mod tests {
    ...
    use super::{IndexResolver, MessageResolver};
