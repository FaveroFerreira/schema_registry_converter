...
use schema_registry_converter::protobuf::SchemaRegistryProtoDeserializer;
...
async fn main() -> anyhow::Result<()> {
    ...
    info!("Starting to consume messages from {}", TOPIC);
...
fn create_consumer() -> anyhow::Result<StreamConsumer> {
    let consumer = ClientConfig::new()
        ...
        .set("group.id", "example-rdkafka-kafka-protobuf-consumer")
...
fn create_deserializer() -> anyhow::Result<SchemaRegistryProtoDeserializer> {
    let sr = Arc::new(CachedSchemaRegistryClient::from_url(
        "http://localhost:8081",
    )?);

    Ok(SchemaRegistryProtoDeserializer::new(sr))
}
